...
!!! note
    gpt-oss-120bは、OpanAI社が公開した、Apache2.0ライセンスで利用可能なモデルです。比較的軽量ながら高い推論能力を提供します。また、日本語にも対応しており、日本語を用いたツール呼び出しの場合に活用可能なモデルです。※（今回のハンズオン環境には含まれませんが、最新版では、Groq Cloud上で高速に動作するgpt-oss-120bをデフォルトで利用することが可能です。）
...
!!! note
    今回のハンズオン環境ではgpt-oss-120bの動作が重く感じると思いますが、これは環境固有の問題で、製品版では快適に動作します。
