...
※AI エージェントのモデルは **GPT-OSS 120B －OpenAI (via Groq)** を使用しています。他のモデルで実行すると結果が異なる場合があります。
...
![alt text](tool_images/image-88.png)  
...
![alt text](tool_images/image-95.png)  

    !!!warning "注意"
        「現在の東京の天気は晴れています。」のように、もっともらしい回答を返すこともありますが、公開されている天気情報などの根拠に基づいていないことに注意してください。
...
![alt text](tool_images/image-89.png)  
...
8. ツールセット では、以下の5つの方法でエージェントにツールを追加するか新規作成できます。

    * カタログから追加 (プリビルドツール)
    * ローカル・インスタンスから追加 (カスタムツール)
    * MCPサーバーからインポート
    * OpenAPIファイルからインポート
    * エージェント型ワークフローを作成する　※ Lab3 で実施します。

    今回は **OpenAPI** をクリックします。  
    ![alt text](tool_images/image-90.png)  

9. 先ほど保存した weather.yaml をアップロードし、**次へ** をクリックします。
![alt text](tool_images/image-64.png)

10. 操作のチェックボックスにチェックを入れ、**完了** をクリックします。  
![alt text](tool_images/image-93.png)

11. 以下のメッセージが表示されたら、ツールの追加は完了です。  
![alt text](tool_images/image-66.png)

## エージェントが出来ることの確認
1. 右側のプレビューチャットに、**あなたのできることを教えてください。** と聞いてみましょう。

2. 様々なことを答えてくれますが、その中に先ほど追加したツールで取得可能な気象情報の提供や、Lab1でアップロードした IBM の年次報告書の提供も含まれていると確認できます。
![alt text](tool_images/image-96.png)

!!!note
    使用しているモデルによっては、1回目の質問では定型の挨拶が回答されることがあります。その場合は、動作に **定型の挨拶は省略し、最初の質問から回答してください。** のように指示を追加すると、1回目から回答してくれるようになります。  
...
以下のように、緯度や経度を確認される場合がありますが、もう一度同じリクエストを入力すると正しく呼び出されるはずです。 
![alt text](tool_images/image-70.png)  

!!!note
    この挙動は、製品が使用しているシステム・プロンプトが、変数の値を勝手に想定、生成するなという指示を与えていることによるものです。
    エージェントの指示やツールの説明を調整することにより、適切な動作を行うように調整することが可能ですが、今回のハンズオンではスコープ外とします。

2. **理由の表示** をクリックして **ステップ1** を開いてみると、入力した地名から緯度と経度を認識し、パラメータを適切に変換してくれていることが確認できます。  
![alt text](tool_images/image-97.png)  


## 複数地名の確認（エージェンティックな振る舞い）
次に、複数の地域の天気を比較する質問をし、エージェンティックな振る舞いを体験してみましょう。

1. チャットをリロードし、**横浜と宇都宮ではどちらの方が涼しい？** と入力してみてください。  
横浜と宇都宮の気温を表示し、どちらの方が涼しいか回答してくれるはずです。  
![alt text](tool_images/image-98.png)  

    上記で記載したシステム・プロンプトの影響により正しく回答されない場合は、横浜と宇都宮の2回分は適切に回答されない可能性が高いため、3回同じ質問をしてみてください。
...
チャットをリロードし、3市町村の緯度・経度ができるだけ異なるように選び、以下のように質問してみてください。  
**例: 函館市、稚内市、羅臼町の気温を表形式で比較してください。**  
![alt text](tool_images/image-99.png)  

こちらも、各市町村の気温はツールによって取得されていますが、LLMがユーザーの指示に従って表形式で出力をしてくれます。
もし表形式で表示されない場合は、**Githubマークダウン形式の表で表示して** とチャットに入力してみましょう。

!!!note
    GPT-OSS-120bは複数項目を表示する際に、表形式で出力する傾向があるため、特に指示をしなくても表形式で出力されることがあります。
...
各エージェントスタイルの概要は、[製品ドキュメント](https://www.ibm.com/docs/ja/watsonx/watson-orchestrate/base?topic=agents-choosing-style-agent)に以下のように記載されています。
-->


???note "エージェントスタイルの違い"

    エージェントスタイルは、AIエージェントがどのように指示を解釈し、ツールを選択し、タスクを完了するかのパターンを選択する機能です。
    エージェント作成画面からはデフォルトまたは ReAct を選択でき、最初はデフォルトで設定されています。  

    各エージェントスタイルの概要は、[製品ドキュメント](https://www.ibm.com/docs/ja/watsonx/watson-orchestrate/base?topic=agents-choosing-style-agent)に以下のように記載されています。ReAct の方が複雑な質問や指示も正しく考えて実行することができますが、その分デフォルトよりも回答が遅くなるため、用途に応じた使い分けが重要です。
...
2. **MCPサーバー** をクリックします。  
![alt text](tool_images/image-92.png)  

3. **MCP サーバーの追加** をクリックします。  
![alt text](tool_images/image-100.png)  

4. 今回は **ローカルMCPサーバー** を選択して **Next** をクリックします。
![alt text](tool_images/image-101.png)  

5. 以下のように入力し、**Import** をクリックします。 (XX は自身のイニシャルを入力します。もし同名のMCPサーバーが既に存在していると表示された場合は、任意の名前に変更してください)  
**サーバー名:** XX_time_mcp  
**インストール・コマンド:** npx -y time-mcp  
![alt text](tool_images/image-102.png)  

6. 接続した Time MCP Server で利用できる外部ツールの一覧が表示されます。  
チェックボックスをクリックすると、エージェントにツールとして追加できます。今回は **current_time** を追加してみましょう。チェックを入れて、**エージェントに追加** をクリックします。  
![alt text](tool_images/image-103.png)    

9. ツールセットを見てみると、先ほど追加した current_time がツールとして表示されています。  
![alt text](tool_images/image-84.png)  

11. 右側のチャットから、追加したツールを呼んでみましょう。  
current_time は現在の日付と時間を教えてくれるため、**ロンドンの現在の日時は何ですか？** のように、お好きな地域の日時を聞いてみてください。  
![alt text](tool_images/image-104.png)  
