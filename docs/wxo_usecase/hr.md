...
??? question "エージェント作成のヒント"
    ...
    ```
    - LLM が質問を上手く理解できない場合や、LLM がナレッジソースではなく独自の回答を含めてしまう場合は、どの情報を使ってどんな処理をすべきエージェントなのか Behavior（動作）に定義することで LLM の挙動を操作します。
    <!-- ※上記に載せているサンプルの会話は、llama-3-2-90b-vision-instruct モデルと ReAct スタイルで実行しています -->
    ```
    [Sample]
    知識ソースを使用して、人事規則に関する質問に回答します。
    ...
    ```
    - また、Knowledge setting（知識設定）の「応答」値の変更も試してみてください。
...
!!! note
    - ツール kintone_get_employeelist は、社員名簿アプリケーションが持つ全社員の個人データ（社員番号や氏名の他、住所やメールアドレスなど）を取得し、社員番号順に並べて出力します。
    - ツール kintone_get_reportline は、社員名簿アプリケーションが持つ全社員の役職情報を取得し、役職順に並べて出力します。
...
??? question "エージェント作成のヒント"
    - 特定ユーザーと同じ組織の情報を出力させるため、組織情報について聞かれた際のデータの検索方法を Behavior（動作）あるいは Guidelines に示します。  
...
!!! note
    - ツール kintone_get_nextoff は、社員名簿アプリケーションから「社員番号」で特定のデータを検索し、次の休暇予定日と共に出力します。  
    ※次の休暇予定日はサンプル値になっています

??? question "エージェント作成のヒント"
    - 対象ユーザー（仮で決めたユーザーの上司）を探し、その上司に対して休暇日取得のツールを実行するよう Behavior（動作）あるいは Guidelines に示します。  
    以下は特定条件のガイドラインとして定義した例です。
...
!!! note
    - ツール kintone_get_timeofflist は、休暇情報アプリケーションから「社員番号」で特定のデータを検索し、休暇データを出力します。
    - ツール kintone_update_timeoff は、休暇情報アプリケーションのデータから「社員番号」で検索した1つのデータに対し、「取得日数」の値を更新（追加）します。実行のためには社員番号の他、新たに取得する日付と日数の値が必要です。  
    ※「残日数」のデータは、「付与日数」「取得日数」の値に応じて休暇情報アプリケーション側で自動計算されます。
